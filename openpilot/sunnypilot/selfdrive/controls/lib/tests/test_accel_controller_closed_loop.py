...
from openpilot.sunnypilot.selfdrive.controls.lib.accel_controller.constants import (
  LEAD_DROPOUT_COAST_TIME, LEAD_RECOVERY_DECEL_RATE, MPC_DECEL_JERK_COST_MULTIPLIER, MPC_DECEL_JERK_MAX_REQUIRED_DECEL,
  MPC_DECEL_JERK_MAX_REQUIRED_DECEL_RATE, MPC_DECEL_TREND_FRAMES, LEAD_RELEASE_CONFIRM_TIME, TARGET_RELEASE_SLEW, STOP_HOLD_EXIT_FRAMES,
...
def _tracked_lead(_current_time: float, lead_name: str, truth: LeadObservation) -> LeadObservation | None:
  return truth | {"radar": True, "radarTrackId": 100} if lead_name == "leadOne" else None
...
class ClosedLoopTrace:
  ...
  raw_speed_ceiling: np.ndarray
  selected_lead: np.ndarray
  lead_recovery: np.ndarray
  ...
  **plant_kwargs,
  ...
  try:
    while plant.current_time < duration:
      ...
      raw_speed_ceiling = controller.lead_controller.raw_speed_ceiling
      ...
      rows.append((
        ...
        controller.launching, controller.departure_launching, controller.output_v_target, raw_speed_ceiling, controller.selected_lead,
        controller.lead_controller.lead_recovery, profile_limit, controller.mpc_accel_max is not None, controller.state,
        controller.required_decel, planner_seed_accel,
        mpc_seed_accel, upper[0], np.min(upper), bounds_valid, plant.planner.mpc.last_solution_status,
      ))
  ...
  trace = ClosedLoopTrace(
    ...
    target_speed=data[:, 11], raw_speed_ceiling=data[:, 12], selected_lead=data[:, 13].astype(int), lead_recovery=data[:, 14].astype(bool),
...
@pytest.mark.parametrize("radar_fresh", (True, False), ids=("fresh-radar", "stale-radar"))
def test_positive_e2e_to_acc_handoff_starts_from_previous_plan(actuator_delay, actuator_lag, radar_fresh):
  plant = Plant(lead_relevancy=False, speed=3.2, actuator_delay=actuator_delay, actuator_lag=actuator_lag)
  _configure_plant(plant, enabled=True, profile=AccelProfile.eco)
  plant.planner._update_radar_freshness = lambda _sm: radar_fresh
  plant.e2e = False
  plant.planner.output_a_target = 0.057
  plant.planner.a_desired = 1.032
  plant.planner.mpc_accel_seed = 1.032
  plant.planner.mpc.source = LongitudinalPlanSource.e2e
  plant.planner.mpc.last_solution_status = 0
  plant.planner._long_active_last_cycle = True

  result = plant.step(v_lead=0.0, v_cruise=15.0)

  assert plant.planner.mpc_accel_seed == pytest.approx(0.057)
  assert abs(result["a_target"] - 0.057) < 0.10
...
def test_clear_road_launch_is_prompt_and_profiles_separate_above_launch_speed():
  ...
  for trace in traces:
    ...
    # The launch ramp has a bounded catch-up step when its speed ceiling is released.
...
@pytest.mark.parametrize(("actuator_delay", "actuator_lag"), ACTUATOR_DYNAMICS, ids=ACTUATOR_IDS)
def test_departure_launch_keeps_profile_ceiling_through_lead_recovery_transition(actuator_delay, actuator_lag):
  trace = _run(
    duration=12.0, controller_enabled=True, profile=AccelProfile.eco, lead_relevancy=True, speed=0.0,
    distance_lead=6.0, v_lead=lambda t: 0.0 if t < 1.0 else min(7.0, 3.0 * (t - 1.0)), v_cruise=12.0,
    actuator_delay=actuator_delay, actuator_lag=actuator_lag, lead_observation_fn=_tracked_lead,
  )
  exits = np.flatnonzero(trace.launching[:-1] & ~trace.launching[1:] & trace.lead_recovery[1:]) + 1

  assert len(exits) == 1
  exit_frame = exits[0]
  response = slice(max(0, exit_frame - 5), min(len(trace.time), exit_frame + 15))
  departure = trace.departure_launching
  assert np.count_nonzero(departure) > 1
  assert trace.accel_ceiling_active[departure].all()
  assert np.min(trace.a_target[response]) > 0.0
  assert np.max(np.abs(np.diff(trace.a_target[response]) / DT_MDL)) < 3.0
  assert not _has_propulsion_brake_cycle(trace.a_target[response])
  assert trace.solver_failures == 0
...
def test_high_speed_lead_seed_release_has_no_target_snap(actuator_delay, actuator_lag):
  ...
  # This lead disappears after settling at a steady speed, rather than while actively restricting.
  # so the trust register's short (persist-frames) pre-roll applies rather than the long dropout
  # coast - see LeadController._dropout_was_restricting
  assert len(release) and trace.time[release[0] + 1] <= dropout_time + LEAD_RELEASE_CONFIRM_TIME + DT_MDL + 1e-9
...
def test_route_537_lead_dropout_does_not_pulse_throttle_before_reacquisition(actuator_delay, actuator_lag):
  ...
  # Allow the slowest actuator model to finish its monotonic release.
...
@pytest.mark.parametrize(("actuator_delay", "actuator_lag"), ACTUATOR_DYNAMICS, ids=ACTUATOR_IDS)
def test_route_554_should_coast_on_dropout_coasts_before_release(actuator_delay, actuator_lag):
  dropout_time = 5.0
...
@pytest.mark.parametrize(("actuator_delay", "actuator_lag"), ACTUATOR_DYNAMICS, ids=ACTUATOR_IDS)
def test_route_575_should_coast_on_dropout_does_not_surge_before_reacquisition(monkeypatch, actuator_delay, actuator_lag):
  ...
  def run(dropout_coast: bool) -> ClosedLoopTrace:
    ...
    def hold_route_target(self, radar_state, *args, **kwargs):
      nonlocal route_frame
      original_update(self, radar_state, *args, **kwargs)
      route_frame += 1
      if route_frame >= target_hold_frame:
        self.lead_controller.target_speed = self.output_v_target = 10.5
      if not dropout_coast and not self.lead_controller.has_lead:
...
def test_lead_bound_routine_decel_uses_smoothing_without_delaying_initial_braking(monkeypatch):
  ...
  # Allow one model frame of timing tolerance.
...
def test_tightening_lead_releases_smoothing_before_late_catchup(monkeypatch):
  ...
  # Releasing smoothing must not brake later than keeping it enabled.
...
def test_same_slot_track_replacement_never_delays_tightening_braking_for_lead():
  event_time = 3.0
  lead_jerk = 1.02
  max_lead_decel = 2.22
  ramp_time = max_lead_decel / lead_jerk
...
def test_stop_hold_rejects_fast_lead_speed_without_range_growth():
  noise_start = 1.0
  distances = (6.000, 6.004, 6.007, 6.010)

  def observe(current_time: float, lead_name: str, truth: LeadObservation) -> LeadObservation | None:
    if lead_name == "leadTwo":
      return None
    frame = round((current_time - noise_start) / DT_MDL)
    if 0 <= frame < len(distances):
      return truth | {"dRel": distances[frame], "vLead": 1.0, "vLeadK": 1.0, "vRel": 1.0,
                      "aLeadK": 0.0, "radarTrackId": 100, "radar": True}
    return truth | {"aLeadK": 0.0, "radarTrackId": 100, "radar": True}
...
def test_filtered_lead_speed_cannot_override_stationary_raw_lead():
  glitch_start = 1.0

  def observe(current_time: float, lead_name: str, truth: LeadObservation) -> LeadObservation | None:
    if lead_name == "leadTwo":
      return None
    frame = max(0, round((current_time - glitch_start) / DT_MDL))
    if current_time >= glitch_start:
      return truth | {"dRel": 6.0 + 0.01 * frame, "vLead": 0.0, "vLeadK": 1.0, "vRel": 0.0,
                      "aLeadK": 0.0, "radarTrackId": 100, "radar": True}
    return truth | {"aLeadK": 0.0, "radarTrackId": 100, "radar": True}
...
@pytest.mark.parametrize(("actuator_delay", "actuator_lag"), ACTUATOR_DYNAMICS, ids=ACTUATOR_IDS)
def test_disappearing_stopped_lead_releases_smoothly(actuator_delay, actuator_lag):
  departure_time = 1.0

  def observe(current_time: float, _lead_name: str, truth: LeadObservation) -> LeadObservation | None:
    return truth if current_time < departure_time else None

  trace = _run(
    duration=3.5, controller_enabled=True, lead_relevancy=True, speed=0.0, distance_lead=6.0,
    v_lead=0.0, v_cruise=8.0, lead_observation_fn=observe,
    actuator_delay=actuator_delay, actuator_lag=actuator_lag,
  )
  released = np.flatnonzero((trace.time >= departure_time) & (trace.target_speed > 0.0))
  moving = np.flatnonzero((trace.time >= departure_time) & (trace.speed > 0.05))
  confirming = (trace.time >= departure_time) & (trace.time < departure_time + LEAD_RELEASE_CONFIRM_TIME - DT_MDL)
  assert len(released) and len(moving)
  assert np.all(trace.target_speed[confirming] == 0.0)

  target_steps = np.diff(trace.target_speed[released[0]:])
  assert np.all(target_steps >= -1e-9)
  assert np.max(target_steps) <= TARGET_RELEASE_SLEW * DT_MDL + 1e-9
  assert trace.time[moving[0]] <= departure_time + LEAD_RELEASE_CONFIRM_TIME + 0.9
...
def test_stop_hold_departure_survives_radar_staleness():
  ...
  common = {
    ...
    "lead_observation_fn": _tracked_lead,
  }
  baseline = _run(**common)
...
def test_confirmed_departure_full_field_dropout_does_not_worsen_stock_response():
  ...
  def observe(current_time: float, lead_name: str, truth: LeadObservation) -> LeadObservation | None:
    ...
    if dropout_start <= current_time < dropout_end:
      ...
      return None
    return truth | {"radar": True, "radarTrackId": 100}
  ...
  assert all(lead_names == {"leadOne"} for lead_names in dropped_by_frame.values())
  assert np.all(trace.selected_lead[dropout] == -1) and np.all(np.isinf(trace.raw_speed_ceiling[dropout]))
...
@pytest.mark.parametrize(("actuator_delay", "actuator_lag"), ACTUATOR_DYNAMICS, ids=ACTUATOR_IDS)
def test_lead_recovery_recovery_preserves_profile_ordering(actuator_delay, actuator_lag):
  traces = [
    _run(
...
def test_high_speed_track_churn_keeps_speed_and_release_bounded():
  trace = _run(
  ...
  # This scenario remains outside lead-recovery mode; assert its closed-loop response.
...
def test_previous_solver_failure_does_not_snap_target():
  common = {
  ...
  # Assert recovery behavior because this scenario remains outside lead-recovery mode.
...
def test_route_507_braking_lead_slot_switch_has_no_false_relief_cycle(profile, actuator_delay, actuator_lag):
  ...
  def observe(current_time: float, lead_name: str, truth: LeadObservation) -> LeadObservation | None:
    if glitch_start <= current_time < glitch_end:
      ...
      return truth | {
        ...
        "vLead": truth["vLead"] + 0.25,
        "vLeadK": truth["vLeadK"] + 0.25,
        "vRel": truth["vRel"] + 0.25,
        "aLeadK": 0.0,
        "radar": True,
        "radarTrackId": 200,
      }
  ...
  assert not _has_propulsion_brake_cycle(trace.a_target[response])
  assert not _has_brake_coast_brake(trace.a_target[response])
  assert np.max(-np.diff(trace.target_speed)[jerk_response]) <= LEAD_RECOVERY_DECEL_RATE * DT_MDL + 1e-9
  assert not trace.fcw.any()
  assert trace.solver_failures == 0
  assert np.max(np.abs(np.diff(trace.a_target)[jerk_response] / DT_MDL)) < 3.0
  assert np.min(gap[response]) >= np.min(clean_gap[response]) - DROPOUT_GAP_TOLERANCE
...
@pytest.mark.parametrize("profile", range(3), ids=("eco", "normal", "sport"))
def test_lead_recovery_dropout_keeps_the_profile_acceleration_ceiling(profile):
  dropout_start = 25.0
  dropout_end = 25.15
...
def test_far_lead_profiles_start_early_in_order_without_solver_failures(actuator_delay, actuator_lag):
  ...
  first_finite_ceilings = [trace.raw_speed_ceiling[np.flatnonzero(np.isfinite(trace.raw_speed_ceiling))[0]] for trace in traces]
  assert first_finite_ceilings[0] < first_finite_ceilings[1] < first_finite_ceilings[2]
  for trace in traces:
...
def test_lead_recovery_slowdown_stays_smooth_without_a_second_braking_stage():
  slowdown_time = 70.0
  ...
  steady_follow = (trace.time >= slowdown_time - 5.0) & (trace.time < slowdown_time)
  response = trace.time >= slowdown_time
  settled = trace.time >= 95.0
  gap = trace.distance_lead - trace.distance
  desired_gap = STOP_DISTANCE + get_T_FOLLOW() * settled_lead_speed

  assert abs(np.mean(trace.speed[steady_follow]) - 10.0) < 0.5
  assert trace.accel_ceiling_active[steady_follow].all()
  np.testing.assert_allclose(trace.mpc_upper_min[steady_follow], trace.profile_accel_max[steady_follow], atol=1e-6)
